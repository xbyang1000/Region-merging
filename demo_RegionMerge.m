function demo_RegionMerge()
clc;
% [filename,filepath] = uigetfile('*.jpg','choose the picture');
...
% % filename = '43.tif';
% %filename = '100007.jpg';
% %filename = 'lena_color.jpg';
%filename = 'polygon_small.tif';     %##
Im = imread(filename);
figure(1);hold on; imshow(Im);
% [row,col,~] = size(Im);
%Im = imresize(Im,[row/4 col/4]);
InitSuperPixelNum = 50;            %##

[label,SPixelNum] = superpixels(Im,InitSuperPixelNum);
BW = boundarymask(label);% backup BW1
BW1 = BW;
[row,col,~] = size(Im);
